...
from diagnosisai.models.classification import ResNet
...
class BasicClassificationPipeline(ABC):
...
class ClassificationPipeline(BasicClassificationPipeline):
    """
    Classification Pipeline based on a specified configuration.

    :param str config_path: Path to the configuration file.
    :param str config_name: Name of the configuration (default is "ResNet").
    :param Optional[str] model_path: Path to the pre-trained model file (default is None).
    """
    def __init__(self, config_path: str, config_name: str = "ResNet", model_path: Optional[str] = None) -> None:
        ...
        if model_path:
            ...
            self.model = ResNet(**self.model_params)
    ...
    def predict(self, test_data: Dataset, batch_size: int = 16) -> tuple[torch.Tensor, dict[str, float]]:
        ...
        for metric_name, metric_func in METRIC_DICT.items():
            ...
            running_metrics[f"{metric_name}"] = metric_func(
                torch.concatenate(all_preds),
                torch.concatenate(all_gts),
                task='multiclass', num_classes=model.num_classes).item()
            
        return all_preds, running_metrics

    def _training_loop(
            self, train_data: Dataset, val_data: Dataset, train_params: dict[str, Any], model: ResNet, loss_func: nn.Module, optimizer, early_stopper,
            scheduler):

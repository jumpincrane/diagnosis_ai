...
# TODO: try with concat in FE block if it fails try with sum
# TODO: apply fe blocks and more concats in decoder
...
class Decoder(nn.Module):
    def __init__(self, out_channels=6, n_filters=64, kernel_size=(3, 3), dropout=0.3, upsample_scale=2, padding=1, batchnorm=False):
        super().__init__()
        self.n_filters = n_filters
        self.kernel_size = kernel_size
    ...
    def forward(self, encFeatures):
        ...
        u7 = t.cat([u7, fe3, fe4], dim=1) # (256, 60, 60) + (256, 60, 60) + (512, 30, 30)= (1024, 60, 60)
        u7 = self.dropout7(u7)
        c7 = self.conv7(u7) # (256, 60, 60)

        u8 = self.convt8(c7) # (128, 60, 60)
        u8 = self.upsample8(u8) # (128, 120, 120)
        u8 = t.cat([u8, fe2, fe3, fe4], dim=1) # (128, 120, 120) + (128, 120, 120) + (256, 60, 60) + (512, 30, 30) = (1024, 120, 120)
        u8 = self.dropout8(u8)
        c8 = self.conv8(u8) # (128, 120, 120)

        u9 = self.convt9(c8) # (64, 120, 120)
        u9 = self.upsample9(u9) # (64, 240, 240)
        u9 = t.cat([u9, fe1, fe2, fe3, fe4], dim=1) # (64, 240, 240) + (64, 240, 240) + (128, 120, 120) + (256, 60, 60) + (512, 30, 30) = (1024, 240, 240)
        u9 = self.dropout9(u9)

...
def calculate_type_errors(pred: t.Tensor, target: t.Tensor, mode: str = 'segment_binary', num_classes: int = None):
    """
    ...
    def __get_conf_atribs__(conf_matrix, num_classes):
        tp_indicies = t.full((num_classes, 1), 0)
        fp_indicies = t.full((num_classes, 1), 1)
        tn_indicies = t.full((num_classes, 1), 2)
        fn_indicies = t.full((num_classes, 1), 3)

        tp = t.take_along_dim(conf_matrix, tp_indicies, dim=1)
        fp = t.take_along_dim(conf_matrix, fp_indicies, dim=1)
        tn = t.take_along_dim(conf_matrix, tn_indicies, dim=1)
        fn = t.take_along_dim(conf_matrix, fn_indicies, dim=1)
    ...
    if mode == "segment_binary":
        # calculate global metrics for all channels, classes (like sample per sample) [B, 1, ...] shape
            tp, fp, tn, fn, _ = stat_scores(pred, target.type(t.int32))
        ...
        for batch in range(batch_size):
            conf_matrix_class = stat_scores(pred[batch], target[batch].type(t.int32), multiclass=True, mdmc_reduce='samplewise', reduce='micro')
        ...
        tp, fp, tn, fn, _ = stat_scores(pred.type(t.int32), target.type(t.int32))
